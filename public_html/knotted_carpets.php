...
<?php if(count($products)===0): ?><p>No products available.</p>
<?php else: foreach($products as $p): ?>
<div class="product-card" data-name="<?php echo htmlspecialchars($p['name']); ?>">
<?php
$imgPath='uploads/knotted carpets/'.$p['image'];
if(!file_exists($imgPath) || empty($p['image'])) $imgPath='https://via.placeholder.com/400x250?text=No+Image';
?>
...
<form method="POST" action="create_order.php" style="display: inline;">
    ...
    <input type="hidden" name="category" value="knotted_carpets">
...
if(alreadyRegistered==="true") window.location.href="Resister.php";
else {localStorage.setItem("alreadyRegistered","true"); window.location.href="Users_login.php";}
}else{
if(this.classList.contains('shop-btn')) window.location.href="product_details.php?id="+pid;
else if(this.classList.contains('cart-btn')) window.location.href="userdashboard.php?action=cart&id="+pid;
}
});
});
});
