...
// Upload directory
$uploadDir = __DIR__ . '/uploads/bamboo bottles/';
...
if(isset($_POST['add_product'])){
    ...
    if($name === '' || $price === ''){
        ...
        header('Location: bamboobottels.php'); exit;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO bamboobottels (name, price, image) VALUES (?, ?, ?)");
...
if(isset($_GET['delete'])){
    ...
    $stmt = $conn->prepare("SELECT image FROM bamboobottels WHERE id=?");
    ...
    if($row){
        $imgName = $row['image'];
        $del = $conn->prepare("DELETE FROM bamboobottels WHERE id=?");
        $del->bind_param("i", $id);
        if($del->execute()){
...
if(isset($_GET['edit'])){
    ...
    $stmt = $conn->prepare("SELECT id,name,price,image FROM bamboobottels WHERE id=?");
...
if(isset($_POST['update_product'])){
    ...
    if($newImage){
        ...
        $upd = $conn->prepare("UPDATE bamboobottels SET name=?, price=?, image=? WHERE id=?");
        ...
        $upd = $conn->prepare("UPDATE bamboobottels SET name=?, price=? WHERE id=?");
...
// ---------------- FETCH ALL PRODUCTS ----------------
$products = [];
$res = $conn->query("SELECT * FROM bamboobottels ORDER BY id DESC");
...
<head>
<meta charset="UTF-8">
<title>Admin - Bamboo Bottles</title>
<style>
...
<p>Current image:</p>
<img src="uploads/bamboo bottles/<?php echo htmlspecialchars($editData['image']); ?>" class="thumb" alt="">
<button type="submit" name="update_product">Update Product</button>
<a href="bamboobottels.php"><button type="button">Cancel</button></a>
</form>
<?php else: ?>
<h2>Add New Bamboo Bottle Product</h2>
<form method="post" enctype="multipart/form-data">
...
<tr>
<td><?php echo $p['id']; ?></td>
<td><img src="uploads/bamboo bottles/<?php echo htmlspecialchars($p['image']); ?>" class="thumb" alt=""></td>
<td><?php echo htmlspecialchars($p['name']); ?></td>
<td>₹<?php echo htmlspecialchars($p['price']); ?></td>
<td class="actions">
<a href="bamboobottels.php?edit=<?php echo $p['id']; ?>">Edit</a>
<a href="bamboobottels.php?delete=<?php echo $p['id']; ?>" onclick="return confirm('Delete this product?')">Delete</a>
</td>
</tr>
