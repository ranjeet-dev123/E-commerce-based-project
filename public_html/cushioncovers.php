...
// Upload directory
$uploadDir = __DIR__ . '/uploads/cushion covers/';
...
if(isset($_POST['add_product'])){
    ...
    if($name === '' || $price === ''){
        ...
        header('Location: cushioncovers.php'); exit;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO cushioncovers (name, price, image) VALUES (?, ?, ?)");
...
if(isset($_GET['delete'])){
    ...
    $stmt = $conn->prepare("SELECT image FROM cushioncovers WHERE id=?");
    ...
    if($row){
        $imgName = $row['image'];
        $del = $conn->prepare("DELETE FROM cushioncovers WHERE id=?");
        $del->bind_param("i", $id);
        if($del->execute()){
...
if(isset($_GET['edit'])){
    ...
    $stmt = $conn->prepare("SELECT id,name,price,image FROM cushioncovers WHERE id=?");
...
if(isset($_POST['update_product'])){
    ...
    if($newImage){
        ...
        $upd = $conn->prepare("UPDATE cushioncovers SET name=?, price=?, image=? WHERE id=?");
        ...
        $upd = $conn->prepare("UPDATE cushioncovers SET name=?, price=? WHERE id=?");
...
// ---------------- FETCH ALL PRODUCTS ----------------
$products = [];
$res = $conn->query("SELECT * FROM cushioncovers ORDER BY id DESC");
...
<head>
<meta charset="UTF-8">
<title>Admin - Cushion Covers</title>
<style>
...
<p>Current image:</p>
<img src="uploads/cushion covers/<?php echo htmlspecialchars($editData['image']); ?>" class="thumb" alt="">
<button type="submit" name="update_product">Update Product</button>
<a href="cushioncovers.php"><button type="button">Cancel</button></a>
</form>
<?php else: ?>
...
<tr>
<td><?php echo $p['id']; ?></td>
<td><img src="uploads/cushion covers/<?php echo htmlspecialchars($p['image']); ?>" class="thumb" alt=""></td>
<td><?php echo htmlspecialchars($p['name']); ?></td>
<td>₹<?php echo htmlspecialchars($p['price']); ?></td>
<td class="actions">
<a href="cushioncovers.php?edit=<?php echo $p['id']; ?>">Edit</a>
<a href="cushioncovers.php?delete=<?php echo $p['id']; ?>" onclick="return confirm('Delete this product?')">Delete</a>
</td>
</tr>
