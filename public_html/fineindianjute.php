...
// Upload directory
$uploadDir = __DIR__ . '/uploads/Fine indian jute/';
...
if(isset($_POST['add_product'])){
    ...
    if($name === '' || $price === ''){
        ...
        header('Location: fineindianjute.php'); exit;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO fineindianjute (name, price, image) VALUES (?, ?, ?)");
...
if(isset($_GET['delete'])){
    ...
    $stmt = $conn->prepare("SELECT image FROM fineindianjute WHERE id=?");
    ...
    if($row){
        $imgName = $row['image'];
        $del = $conn->prepare("DELETE FROM fineindianjute WHERE id=?");
        $del->bind_param("i", $id);
        if($del->execute()){
...
if(isset($_GET['edit'])){
    ...
    $stmt = $conn->prepare("SELECT id,name,price,image FROM fineindianjute WHERE id=?");
...
if(isset($_POST['update_product'])){
    ...
    if($newImage){
        ...
        $upd = $conn->prepare("UPDATE fineindianjute SET name=?, price=?, image=? WHERE id=?");
        ...
        $upd = $conn->prepare("UPDATE fineindianjute SET name=?, price=? WHERE id=?");
...
// ---------------- FETCH ALL PRODUCTS ----------------
$products = [];
$res = $conn->query("SELECT * FROM fineindianjute ORDER BY id DESC");
...
<head>
<meta charset="UTF-8">
<title>Admin - Fine India Jute</title>
<style>
...
<h1>Fine India Jute Products</h1>
...
<p>Current image:</p>
<img src="uploads/Fine indian jute/<?php echo htmlspecialchars($editData['image']); ?>" class="thumb" alt="">
<button type="submit" name="update_product">Update Product</button>
<a href="fineindianjute.php"><button type="button">Cancel</button></a>
</form>
<?php else: ?>
<h2>Add New Fine India Jute Product</h2>
<form method="post" enctype="multipart/form-data">
...
<tr>
<td><?php echo $p['id']; ?></td>
<td><img src="uploads/Fine indian jute/<?php echo htmlspecialchars($p['image']); ?>" class="thumb" alt=""></td>
<td><?php echo htmlspecialchars($p['name']); ?></td>
<td>₹<?php echo htmlspecialchars($p['price']); ?></td>
<td class="actions">
<a href="fineindianjute.php?edit=<?php echo $p['id']; ?>">Edit</a>
<a href="fineindianjute.php?delete=<?php echo $p['id']; ?>" onclick="return confirm('Delete this product?')">Delete</a>
</td>
</tr>
