...
// Upload directory
$uploadDir = __DIR__ . '/uploads/Fine indian durrys/';
...
if(isset($_POST['add_product'])){
    ...
    if($name === '' || $price === ''){
        ...
        header('Location: fineindiandurrys.php'); exit;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO fineindiarugs (name, price, image) VALUES (?, ?, ?)");
...
if(isset($_GET['delete'])){
    ...
    $stmt = $conn->prepare("SELECT image FROM fineindiarugs WHERE id=?");
    ...
    if($row){
        $imgName = $row['image'];
        $del = $conn->prepare("DELETE FROM fineindiarugs WHERE id=?");
        $del->bind_param("i", $id);
        if($del->execute()){
...
if(isset($_GET['edit'])){
    ...
    $stmt = $conn->prepare("SELECT id,name,price,image FROM fineindiarugs WHERE id=?");
...
if(isset($_POST['update_product'])){
    ...
    if($newImage){
        ...
        $upd = $conn->prepare("UPDATE fineindiarugs SET name=?, price=?, image=? WHERE id=?");
        ...
        $upd = $conn->prepare("UPDATE fineindiarugs SET name=?, price=? WHERE id=?");
...
// ---------------- FETCH ALL PRODUCTS ----------------
$products = [];
$res = $conn->query("SELECT * FROM fineindiarugs ORDER BY id DESC");
...
<head>
<meta charset="UTF-8">
<title>Admin - Fine India Durrys</title>
<style>
...
<h1>Fine India Durrys Products</h1>
...
<p>Current image:</p>
<img src="uploads/Fine indian durrys/<?php echo htmlspecialchars($editData['image']); ?>" class="thumb" alt="">
<button type="submit" name="update_product">Update Product</button>
<a href="fineindiandurrys.php"><button type="button">Cancel</button></a>
</form>
<?php else: ?>
<h2>Add New Fine India Durrys Product</h2>
<form method="post" enctype="multipart/form-data">
...
<tr>
<td><?php echo $p['id']; ?></td>
<td><img src="uploads/Fine indian durrys/<?php echo htmlspecialchars($p['image']); ?>" class="thumb" alt=""></td>
<td><?php echo htmlspecialchars($p['name']); ?></td>
<td>₹<?php echo htmlspecialchars($p['price']); ?></td>
<td class="actions">
<a href="fineindiandurrys.php?edit=<?php echo $p['id']; ?>">Edit</a>
<a href="fineindiandurrys.php?delete=<?php echo $p['id']; ?>" onclick="return confirm('Delete this product?')">Delete</a>
</td>
</tr>
