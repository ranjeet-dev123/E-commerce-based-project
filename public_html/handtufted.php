...
// ensure uploads/Handtufted dir exists
$uploadDir = __DIR__ . '/uploads/Handtufted/';
...
if(isset($_POST['add_product'])){
    ...
    if($name === '' || $price === ''){
        ...
        header('Location: handtufted.php'); exit;
    }
    ...
    // INSERT into handtufted table
    $stmt = $conn->prepare("INSERT INTO handtufted (name, price, image) VALUES (?, ?, ?)");
...
if(isset($_GET['delete'])){
    ...
    $stmt = $conn->prepare("SELECT image FROM handtufted WHERE id=?");
    $stmt->bind_param("i",$id);
    $stmt->execute();
    $stmt->bind_result($imgName);
    if($stmt->fetch()){
        $stmt->close();
        $del = $conn->prepare("DELETE FROM handtufted WHERE id=?");
        $del->bind_param("i",$id);
        if($del->execute()){
...
if(isset($_GET['edit'])){
    ...
    $stmt = $conn->prepare("SELECT id,name,price,image FROM handtufted WHERE id=? ");
...
if(isset($_POST['update_product'])){
    ...
    if($newImage){
        ...
        $upd = $conn->prepare("UPDATE handtufted SET name=?, price=?, image=? WHERE id=?");
        ...
        $upd = $conn->prepare("UPDATE handtufted SET name=?, price=? WHERE id=?");
...
// ---------------- FETCH ALL PRODUCTS ----------------
$products = [];
$res = $conn->query("SELECT * FROM handtufted ORDER BY id DESC");
...
<form method="post" enctype="multipart/form-data">
    ...
    <p>Current image:</p>
    <img src="uploads/Handtufted/<?php echo htmlspecialchars($editData['image']); ?>" class="thumb" alt="">
    <button type="submit" name="update_product">Update Product</button>
    <a href="handtufted.php"><button type="button">Cancel</button></a>
</form>
<?php else: ?>
<h2>Add New Handtufted Product</h2>
<form method="post" enctype="multipart/form-data">
...
<table>
    ...
    <?php else: foreach($products as $p): ?>
        <tr>
            <td><?php echo $p['id']; ?></td>
            <td><img src="uploads/Handtufted/<?php echo htmlspecialchars($p['image']); ?>" class="thumb" alt=""></td>
            <td><?php echo htmlspecialchars($p['name']); ?></td>
            <td>₹<?php echo htmlspecialchars($p['price']); ?></td>
            <td class="actions">
                <a href="handtufted.php?edit=<?php echo $p['id']; ?>">Edit</a>
                <a href="handtufted.php?delete=<?php echo $p['id']; ?>" onclick="return confirm('Delete this product?')">Delete</a>
            </td>
        </tr>
