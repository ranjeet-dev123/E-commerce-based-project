...
// Upload directory
$uploadDir = __DIR__ . '/uploads/organic color dying wollen/';
...
if(isset($_POST['add_product'])){
    ...
    if($name === '' || $price === ''){
        ...
        header('Location: organicyarns.php'); exit;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO organicyarns (name, price, image) VALUES (?, ?, ?)");
...
if(isset($_GET['delete'])){
    ...
    $stmt = $conn->prepare("SELECT image FROM organicyarns WHERE id=?");
    ...
    if($row){
        $imgName = $row['image'];
        $del = $conn->prepare("DELETE FROM organicyarns WHERE id=?");
        $del->bind_param("i", $id);
        if($del->execute()){
...
if(isset($_GET['edit'])){
    ...
    $stmt = $conn->prepare("SELECT id,name,price,image FROM organicyarns WHERE id=?");
...
if(isset($_POST['update_product'])){
    ...
    if($newImage){
        ...
        $upd = $conn->prepare("UPDATE organicyarns SET name=?, price=?, image=? WHERE id=?");
        ...
        $upd = $conn->prepare("UPDATE organicyarns SET name=?, price=? WHERE id=?");
...
// ---------------- FETCH ALL PRODUCTS ----------------
$products = [];
$res = $conn->query("SELECT * FROM organicyarns ORDER BY id DESC");
...
<head>
<meta charset="UTF-8">
<title>Admin - Organic Yarns</title>
<style>
...
<p>Current image:</p>
<img src="uploads/organic color dying wollen/<?php echo htmlspecialchars($editData['image']); ?>" class="thumb" alt="">
<button type="submit" name="update_product">Update Product</button>
<a href="organicyarns.php"><button type="button">Cancel</button></a>
</form>
<?php else: ?>
...
<tr>
<td><?php echo $p['id']; ?></td>
<td><img src="uploads/organic color dying wollen/<?php echo htmlspecialchars($p['image']); ?>" class="thumb" alt=""></td>
<td><?php echo htmlspecialchars($p['name']); ?></td>
<td>₹<?php echo htmlspecialchars($p['price']); ?></td>
<td class="actions">
<a href="organicyarns.php?edit=<?php echo $p['id']; ?>">Edit</a>
<a href="organicyarns.php?delete=<?php echo $p['id']; ?>" onclick="return confirm('Delete this product?')">Delete</a>
</td>
</tr>
