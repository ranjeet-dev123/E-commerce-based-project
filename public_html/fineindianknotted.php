...
// Upload directory
$uploadDir = __DIR__ . '/uploads/Fine indian knotted/';
...
if(isset($_POST['add_product'])){
    ...
    if($name === '' || $price === ''){
        ...
        header('Location: fineindianknotted.php'); exit;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO fineindianknotted (name, price, image) VALUES (?, ?, ?)");
...
if(isset($_GET['delete'])){
    ...
    $stmt = $conn->prepare("SELECT image FROM fineindianknotted WHERE id=?");
    ...
    if($row){
        $imgName = $row['image'];
        $del = $conn->prepare("DELETE FROM fineindianknotted WHERE id=?");
        $del->bind_param("i", $id);
        if($del->execute()){
...
if(isset($_GET['edit'])){
    ...
    $stmt = $conn->prepare("SELECT id,name,price,image FROM fineindianknotted WHERE id=?");
...
if(isset($_POST['update_product'])){
    ...
    if($newImage){
        ...
        $upd = $conn->prepare("UPDATE fineindianknotted SET name=?, price=?, image=? WHERE id=?");
        ...
        $upd = $conn->prepare("UPDATE fineindianknotted SET name=?, price=? WHERE id=?");
...
// ---------------- FETCH ALL PRODUCTS ----------------
$products = [];
$res = $conn->query("SELECT * FROM fineindianknotted ORDER BY id DESC");
...
<head>
<meta charset="UTF-8">
<title>Admin - Fine India Knotted</title>
<style>
...
<h1>Fine India Knotted Products</h1>
...
<p>Current image:</p>
<img src="uploads/Fine indian knotted/<?php echo htmlspecialchars($editData['image']); ?>" class="thumb" alt="">
<button type="submit" name="update_product">Update Product</button>
<a href="fineindianknotted.php"><button type="button">Cancel</button></a>
</form>
<?php else: ?>
<h2>Add New Fine India Knotted Product</h2>
<form method="post" enctype="multipart/form-data">
...
<tr>
<td><?php echo $p['id']; ?></td>
<td><img src="uploads/Fine indian knotted/<?php echo htmlspecialchars($p['image']); ?>" class="thumb" alt=""></td>
<td><?php echo htmlspecialchars($p['name']); ?></td>
<td>₹<?php echo htmlspecialchars($p['price']); ?></td>
<td class="actions">
<a href="fineindianknotted.php?edit=<?php echo $p['id']; ?>">Edit</a>
<a href="fineindianknotted.php?delete=<?php echo $p['id']; ?>" onclick="return confirm('Delete this product?')">Delete</a>
</td>
</tr>
