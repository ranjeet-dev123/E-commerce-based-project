<?php
session_start();
include 'config.php'; // config.php me aapka database 'rugs' connected hona chahiye

// ensure uploads/Fashion Flooring dir exists
$uploadDir = __DIR__ . '/uploads/Fashion Flooring/';
...
if(isset($_POST['add_product'])){
    ...
    if($name === '' || $price === ''){
        ...
        header('Location: FashionFlooring.php'); exit;
    }
    ...
    if(!move_uploaded_file($_FILES['image']['tmp_name'],$dest)){
        flash('Failed to move uploaded image.');
        header('Location: FashionFlooring.php'); exit;
    }

    // INSERT into fashionflooring table
    $stmt = $conn->prepare("INSERT INTO fashionflooring (name, price, image) VALUES (?, ?, ?)");
...
if(isset($_GET['delete'])){
    ...
    $stmt = $conn->prepare("SELECT image FROM fashionflooring WHERE id=?");
    $stmt->bind_param("i",$id);
    $stmt->execute();
    $stmt->bind_result($imgName);
    if($stmt->fetch()){
        $stmt->close();
        $del = $conn->prepare("DELETE FROM fashionflooring WHERE id=?");
        $del->bind_param("i",$id);
        if($del->execute()){
...
if(isset($_GET['edit'])){
    ...
    $stmt = $conn->prepare("SELECT id,name,price,image FROM fashionflooring WHERE id=? ");
...
if(isset($_POST['update_product'])){
    ...
    if($newImage){
        ...
        $upd = $conn->prepare("UPDATE fashionflooring SET name=?, price=?, image=? WHERE id=?");
        ...
        $upd = $conn->prepare("UPDATE fashionflooring SET name=?, price=? WHERE id=?");
...
// ---------------- FETCH ALL PRODUCTS ----------------
$products = [];
$res = $conn->query("SELECT * FROM fashionflooring  ORDER BY id DESC");
...
<head>
<meta charset="UTF-8">
<title>Admin - Fashion Flooring</title>
<style>
...
<h1>Fashion Flooring Products</h1>
...
<form method="post" enctype="multipart/form-data">
    ...
    <p>Current image:</p>
    <img src="uploads/Fashion Flooring/<?php echo htmlspecialchars($editData['image']); ?>" class="thumb" alt="">
    <button type="submit" name="update_product">Update Product</button>
    <a href="FashionFlooring.php"><button type="button">Cancel</button></a>
</form>
<?php else: ?>
<h2>Add New Fashion Flooring Product</h2>
<form method="post" enctype="multipart/form-data">
...
<h2>All Fashion Flooring Products</h2>
<table>
    <thead>
    ...
    <?php else: foreach($products as $p): ?>
        <tr>
            <td><?php echo $p['id']; ?></td>
            <td><img src="uploads/Fashion Flooring/<?php echo htmlspecialchars($p['image']); ?>" class="thumb" alt=""></td>
            <td><?php echo htmlspecialchars($p['name']); ?></td>
            <td>₹<?php echo htmlspecialchars($p['price']); ?></td>
            <td class="actions">
                <a href="FashionFlooring.php?edit=<?php echo $p['id']; ?>">Edit</a>
                <a href="FashionFlooring.php?delete=<?php echo $p['id']; ?>" onclick="return confirm('Delete this product?')">Delete</a>
            </td>
        </tr>
