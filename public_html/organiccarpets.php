...
// Upload directory
$uploadDir = __DIR__ . '/uploads/organic color dyning handmade/';
...
if(isset($_POST['add_product'])){
    ...
    if($name === '' || $price === ''){
        ...
        header('Location: organiccarpets.php'); exit;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO organiccarpets (name, price, image) VALUES (?, ?, ?)");
...
if(isset($_GET['delete'])){
    ...
    $stmt = $conn->prepare("SELECT image FROM organiccarpets WHERE id=?");
    ...
    if($row){
        $imgName = $row['image'];
        $del = $conn->prepare("DELETE FROM organiccarpets WHERE id=?");
        $del->bind_param("i", $id);
        if($del->execute()){
...
if(isset($_GET['edit'])){
    ...
    $stmt = $conn->prepare("SELECT id,name,price,image FROM organiccarpets WHERE id=?");
...
if(isset($_POST['update_product'])){
    ...
    if($newImage){
        ...
        $upd = $conn->prepare("UPDATE organiccarpets SET name=?, price=?, image=? WHERE id=?");
        ...
        $upd = $conn->prepare("UPDATE organiccarpets SET name=?, price=? WHERE id=?");
...
// ---------------- FETCH ALL PRODUCTS ----------------
$products = [];
$res = $conn->query("SELECT * FROM organiccarpets ORDER BY id DESC");
...
<head>
<meta charset="UTF-8">
<title>Admin - Organic Carpets</title>
<style>
...
<h1>Organic Carpets Products</h1>
...
<p>Current image:</p>
<img src="uploads/organic color dyning handmade/<?php echo htmlspecialchars($editData['image']); ?>" class="thumb" alt="">
<button type="submit" name="update_product">Update Product</button>
<a href="organiccarpets.php"><button type="button">Cancel</button></a>
</form>
<?php else: ?>
<h2>Add New Organic Carpet Product</h2>
<form method="post" enctype="multipart/form-data">
...
<tr>
<td><?php echo $p['id']; ?></td>
<td><img src="uploads/organic color dyning handmade/<?php echo htmlspecialchars($p['image']); ?>" class="thumb" alt=""></td>
<td><?php echo htmlspecialchars($p['name']); ?></td>
<td>₹<?php echo htmlspecialchars($p['price']); ?></td>
<td class="actions">
<a href="organiccarpets.php?edit=<?php echo $p['id']; ?>">Edit</a>
<a href="organiccarpets.php?delete=<?php echo $p['id']; ?>" onclick="return confirm('Delete this product?')">Delete</a>
</td>
</tr>
