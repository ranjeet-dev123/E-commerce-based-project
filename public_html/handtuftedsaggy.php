...
// Upload directory
$uploadDir = __DIR__ . '/uploads/Hand tufted& saggy/';
...
if(isset($_POST['add_product'])){
    ...
    if($name === '' || $price === ''){
        ...
        header('Location: handtuftedsaggy.php'); exit;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO handtuftedsaggy (name, price, image) VALUES (?, ?, ?)");
...
if(isset($_GET['delete'])){
    ...
    $stmt = $conn->prepare("SELECT image FROM handtuftedsaggy WHERE id=?");
    ...
    if($row){
        $imgName = $row['image'];
        $del = $conn->prepare("DELETE FROM handtuftedsaggy WHERE id=?");
        $del->bind_param("i", $id);
        if($del->execute()){
...
if(isset($_GET['edit'])){
    ...
    $stmt = $conn->prepare("SELECT id,name,price,image FROM handtuftedsaggy WHERE id=?");
...
if(isset($_POST['update_product'])){
    ...
    if($newImage){
        ...
        $upd = $conn->prepare("UPDATE handtuftedsaggy SET name=?, price=?, image=? WHERE id=?");
        ...
        $upd = $conn->prepare("UPDATE handtuftedsaggy SET name=?, price=? WHERE id=?");
...
// ---------------- FETCH ALL PRODUCTS ----------------
$products = [];
$res = $conn->query("SELECT * FROM handtuftedsaggy ORDER BY id DESC");
...
<head>
<meta charset="UTF-8">
<title>Admin - Hand Tufted & Saggy</title>
<style>
...
<h1>Hand Tufted & Saggy Products</h1>
...
<p>Current image:</p>
<img src="uploads/Hand tufted& saggy/<?php echo htmlspecialchars($editData['image']); ?>" class="thumb" alt="">
<button type="submit" name="update_product">Update Product</button>
<a href="handtuftedsaggy.php"><button type="button">Cancel</button></a>
</form>
<?php else: ?>
...
<tr>
<td><?php echo $p['id']; ?></td>
<td><img src="uploads/Hand tufted& saggy/<?php echo htmlspecialchars($p['image']); ?>" class="thumb" alt=""></td>
<td><?php echo htmlspecialchars($p['name']); ?></td>
<td>₹<?php echo htmlspecialchars($p['price']); ?></td>
<td class="actions">
<a href="handtuftedsaggy.php?edit=<?php echo $p['id']; ?>">Edit</a>
<a href="handtuftedsaggy.php?delete=<?php echo $p['id']; ?>" onclick="return confirm('Delete this product?')">Delete</a>
</td>
</tr>
