...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(empty($Name) || empty($Mobile) || empty($Email) || empty($Message)){
        ...
        // Duplicate check
        $check = "SELECT id FROM contactme WHERE Email = '$Email'";
        $res = mysqli_query($conn, $check);

        if(mysqli_num_rows($res) > 0){
            echo "<script>alert('You have already submitted a message with this Email!');</script>";
        } else {
            // Insert data
            $sql = "INSERT INTO contactme (Name, Mobile, Email, Message) 
                    VALUES ('$Name', '$Mobile', '$Email', '$Message')";
            if (mysqli_query($conn, $sql)) {
                echo "<script>alert(' Submitted successfully! Thank you.');</script>";
            } else {
...
<div class="product-container">
  ...
  <?php else: foreach ($products as $p): ?>
    <div class="product-card" data-name="<?php echo htmlspecialchars($p['name']); ?>">
      <?php
      $imgPath = 'uploads/bamboo bottles/' . $p['image'];
    ...
    <form method="POST" action="create_order.php" style="display: inline;">
        ...
        <input type="hidden" name="category" value="bamboo_bottels">
