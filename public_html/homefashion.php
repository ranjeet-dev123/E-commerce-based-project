...
// Upload directory
$uploadDir = __DIR__ . '/uploads/Home fashion flooring/';
...
if(isset($_POST['add_product'])){
    ...
    if($name === '' || $price === ''){
        ...
        header('Location: homefashion.php'); exit;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO homefashion (name, price, image) VALUES (?, ?, ?)");
...
if(isset($_GET['delete'])){
    ...
    $stmt = $conn->prepare("SELECT image FROM homefashion WHERE id=?");
    ...
    if($row){
        $imgName = $row['image'];
        $del = $conn->prepare("DELETE FROM homefashion WHERE id=?");
        $del->bind_param("i", $id);
        if($del->execute()){
...
if(isset($_GET['edit'])){
    ...
    $stmt = $conn->prepare("SELECT id,name,price,image FROM homefashion WHERE id=?");
...
if(isset($_POST['update_product'])){
    ...
    if($newImage){
        ...
        $upd = $conn->prepare("UPDATE homefashion SET name=?, price=?, image=? WHERE id=?");
        ...
        $upd = $conn->prepare("UPDATE homefashion SET name=?, price=? WHERE id=?");
...
$products = [];
$res = $conn->query("SELECT * FROM homefashion ORDER BY id DESC");
...
<head>
<meta charset="UTF-8">
<title>Admin - Home Fashion Flooring</title>
<style>
...
<h1>Home Fashion Flooring Products</h1>
...
<p>Current image:</p>
<img src="uploads/Home fashion flooring/<?php echo htmlspecialchars($editData['image']); ?>" class="thumb" alt="">
<button type="submit" name="update_product">Update Product</button>
<a href="homefashion.php"><button type="button">Cancel</button></a>
</form>
<?php else: ?>
...
<tr>
<td><?php echo $p['id']; ?></td>
<td><img src="uploads/Home fashion flooring/<?php echo htmlspecialchars($p['image']); ?>" class="thumb" alt=""></td>
<td><?php echo htmlspecialchars($p['name']); ?></td>
<td>₹<?php echo htmlspecialchars($p['price']); ?></td>
<td class="actions">
<a href="homefashion.php?edit=<?php echo $p['id']; ?>">Edit</a>
<a href="homefashion.php?delete=<?php echo $p['id']; ?>" onclick="return confirm('Delete this product?')">Delete</a>
</td>
</tr>
